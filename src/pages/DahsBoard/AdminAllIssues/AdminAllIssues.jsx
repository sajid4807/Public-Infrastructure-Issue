...
const AdminAllIssues = () => {
  ...
  const { data: issuesData = {}, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/reports/admin?page=${page}&limit=${limit}`
      );
      return res.data
    },
  ...
  const handleAssignStaff = async (e) => {
    e.preventDefault();
    if (!selectedStaff) return Swal.fire("Error", "Select a staff", "error");
                  ...
                  {issues.length === 0 ? (
                    ...
                    issues.map((issue) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs capitalize font-bold ${
                          ...
                          >
                            ...
                            {issue.status === "in-progress" && <FaClock />}
                            {issue.status === "working" && <FaCog />}
                            {issue.status === "resolved" && <FaCheckCircle />}
                            {issue.status === "rejected" && <FaTimesCircle />}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold capitalize text-white text-lg">
                            {issue.title}
                          </h3>
