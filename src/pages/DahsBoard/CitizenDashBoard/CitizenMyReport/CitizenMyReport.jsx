...
const CitizenMyReport = () => {
  ...
  if (result.isConfirmed) {
    axiosSecure.delete(`/reports/${issueId}`)
    .then(()=>{
        Swal.fire({
      title: "Deleted!",
  ...
  <div className="mb-6">
    ...
    <p className="text-sm font-bold text-gray-500">Total Report Issue: {issues.length}</p>
      ...
      <tbody>
        {issues.map((issue,index) => (
          <tr key={issue._id} className="hover:bg-gray-50">
            ...
            <td className="text-center font-bold capitalize">{issue.title}</td>
            <td className="text-center font-medium capitalize">{issue.category}</td>
            ...
            <td className="text-center">{new Date(issue.createdAt).toLocaleDateString()}</td>
