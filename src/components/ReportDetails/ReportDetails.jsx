      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-3 md:mb-8">
          <div className="flex items-center gap-3 mb-2 md:mb-4">
            <button
              onClick={() => navigate(-1)}
              className="p-2 rounded-lg bg-white/70 backdrop-blur-xl border border-white/20 hover:bg-white transition-all"
            >
              <svg className="w-6 h-6 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <h1 className="text-xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 capitalize">
              {issue.title}
            </h1>
          </div>
        ...
        {/* Image Card */}
        <div className="relative group mb-4 md:mb-8 rounded-xl overflow-hidden shadow-lg">
          <img
            src={issue.imageURL}
            alt="Issue"
          ...
          {[
            {
              ...
              icon: (
                <svg className="w-3 md:w-6 h-3 md:h-6" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div
              key={idx}
              className="group text-center md:text-start relative overflow-hidden rounded-xl bg-white/70 backdrop-blur-xl border border-white/20 p-2 md:p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
            ...
            >
              ...
              <div className="relative z-10 space-y-1 md:space-y-3">
                <div className={`inline-flex p-3 bg-gradient-to-br ${item.gradient} rounded-lg text-white shadow-lg`}>
                  {item.icon}
                </div>
                <p className="text-sm font-bold text-slate-600 uppercase tracking-wide">{item.label}</p>
                {item.badge ? (
                  <span className={`inline-block px-4 py-2 bg-gradient-to-r ${item.gradient} text-white font-bold text-sm rounded-lg shadow-lg capitalize`}>
                    {item.value}
                  </span>
                ) : (
        ...
        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row justify-between gap-4 mb-2 md:mb-8">
          <div className="flex flex-wrap gap-2 md:gap-3">
            {user &&
              ...
              issue.status?.toLowerCase() === "pending" && (
                <Link
                  ...
                  className="flex items-center gap-2 px-2 md:px-6 py-2 md:py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-medium md:font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
                >
            ...
            {user && issue.email === user.email && (
              <button
                ...
                className="flex items-center gap-2 px-2 md:px-6 py-2 md:py-3 bg-gradient-to-r from-red-600 to-pink-600 text-white font-medium md:font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {user &&
              ...
              issue.priority === "normal" && (
                <button
                  onClick={handleBoost}
                  className="flex items-center gap-2 px-2 md:px-6 py-2 md:py-3 bg-gradient-to-r from-orange-600 to-red-600 text-white font-medium md:font-bold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Description Card */}
        <div className="relative overflow-hidden rounded-xl bg-white/70 backdrop-blur-xl border border-white/20 p-3 md:p-8 shadow-lg">
